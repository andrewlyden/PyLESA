...
class Lorentz(PerformanceModel):
    ...
    def __init__(
        self,
        cop_spec: float,
        flow_temp_spec: float,
        return_temp_spec: float,
        ambient_temp_in_spec: float,
        ambient_temp_out_spec: float,
        elec_capacity: float,
    ):
        ...
        self._set_efficiency(
            ...
            flow_temp_spec,
            return_temp_spec,
            ambient_temp_in_spec,
            ambient_temp_out_spec,
        )
    ...
    def _cop_lorentz(
        self,
        flow_temp: float | np.ndarray[float],
        return_temp: float | np.ndarray[float],
        ambient_temp_in: float | np.ndarray[float],
        ambient_temp_out: float | np.ndarray[float],
    ) -> float | np.ndarray[float]:
        """Lorentz COP calculation based on EnergyPRO method"""
        t_high_mean = (flow_temp - return_temp) / (
            np.log((flow_temp + ABS_ZERO) / (return_temp + ABS_ZERO))
        )

        t_low_mean = (ambient_temp_in - ambient_temp_out) / (
            np.log((ambient_temp_in + ABS_ZERO) / (ambient_temp_out + ABS_ZERO))
        )
    ...
    def _set_efficiency(
        ...
        self._efficiency = cop_spec / self._cop_lorentz(
            flow_temp_spec,
            return_temp_spec,
            ambient_temp_in_spec,
            ambient_temp_out_spec,
    ...
    def cop(
        self,
        flow_temp: np.ndarray[float],
        return_temp: np.ndarray[float],
        ambient_temp_in: np.ndarray[float],
        ambient_temp_out: np.ndarray[float],
    ) -> np.ndarray[float]:
        ...
        Returns:
            Cop for timestep
        """
        return self.efficiency * self._cop_lorentz(
            flow_temp, return_temp, ambient_temp_in, ambient_temp_out
        )
